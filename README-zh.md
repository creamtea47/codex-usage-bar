...
- 长周期额度显示控量线；悬停提示会说明它是按额度窗口已过去的时间计算出的“建议最低剩余额度”，仅用于本地节奏参考，不是官方阈值。本地样本足够时，可靠的耗尽预测可替换该控量提示。
...
- 设置以独立、不透明的原生窗口打开，左侧分为“显示”“数据与刷新”“趋势”“启动”“关于与更新”五类。
- 提供可选的系统通知：低剩余额度、消耗快于时间进度和额度周期重置。总开关默认关闭，三个子项默认开启；低额度初值为 20%，超速差值初值为 10 个百分点，两者都支持 0–100。静默时段默认关闭，初始范围为 22:00–08:00，支持跨午夜，期间抑制的事件不会在结束后补发。
- 在设置中提供独立的 24 小时 / 7 天趋势页。采集默认开启，仅保存到本机 `usage-history.json`，可暂停或清除；检测到账号指纹变化时立即清空旧账号历史。每个额度窗口显示固定 0–100% 图表、重置周期断线、近段消耗和四态本地预测。
...
打开后，卡片会立即尝试读取用量。右上角“刷新”可立即手动重试，即使当前正等待失败退避也不受影响；旁边的“设置”会打开独立窗口，其中按“显示”“数据与刷新”“趋势”“启动”“关于与更新”分类。

“显示”页可切换语言、主题、置顶和位置 / 大小锁定；“数据与刷新”页设置固定刷新间隔和系统通知规则；“趋势”页切换 24 小时 / 7 天、控制本地采集并提供二次确认的清除操作；“启动”页控制开机自启；“关于与更新”页显示当前版本、更新操作、脱敏诊断、问题反馈和更新说明。关闭设置窗口只会隐藏设置页，悬浮卡会继续运行；关闭悬浮卡才会退出应用。
...
趋势采集对新安装和升级用户都默认开启，只记录成功快照，最多保留七天，并在重置周期之间断线。预测仅使用当前周期的近期样本，显示“正在采集”“近期稳定”“预计在重置前耗尽”或“预计可撑到重置”；可靠的耗尽时刻按 15 分钟取整，并且不会外推到重置之后。关闭采集只停止新增样本，不会上传任何数据；“清除历史”会在二次确认后永久删除已有曲线点。

“复制脱敏诊断”只把白名单摘要写入剪贴板。“反馈问题”会单独打开 GitHub Issue 页面，绝不会附加诊断；请先检查复制内容，再自行决定是否粘贴。“查看更新说明”会为合法的 `X.Y.Z` 应用版本打开对应 tag 页面，版本异常时回退到仓库 Releases 总页。
...
认证文件只由 Rust 后端在本地请求期间读取。React 前端仅收到经过过滤的状态、套餐标签、刷新时间、稳定错误代码、额度窗口数据，以及成功用量响应中存在时的掩码邮箱。前端会本地翻译认证缺失 / 失效、网络、限流、服务异常、响应格式异常和本地桥接异常；未知代码使用安全的通用提示。原始邮箱会先在 Rust 中转换为如 `j***@example.com` 的形式再发送到界面；前端从不接收 `auth.json`、access token、请求头、接口原始响应、原始错误或未掩码邮箱。
...
- 诊断摘要采用固定白名单：schema 与应用版本、平台 / 架构、语言、主题、刷新间隔、连续失败次数、通知开关 / 权限、Dashboard 状态 / 错误代码、窗口数、最近刷新、更新状态、历史样本数和存储状态。明确排除 Token、账号、邮箱、路径、代理、URL、额度百分比、重置时间、曲线点、原始错误和日志。
- 设置 WebView 只有剪贴板**写文本**权限，没有读取权限；外链仅允许当前仓库的 Release 和新建 Issue 页面。主 WebView 没有剪贴板或通知插件权限；通知权限申请与测试都必须经过设置窗口专属 Rust 命令。
...
| 原生边界 | Tauri 2 command / event / capability | 按窗口限制 IPC、系统通知权限与发送、只写剪贴板，以及仓库 Issue / Release 专属外链 |
| 数据与持久化 | Rust + Tokio + Reqwest + Serde | 只读认证、固定刷新与失败退避、请求去重、内存快照、版本化本地历史、预测、设置、自启和脱敏日志 |

完整前端 IPC 为 `get_dashboard`、`refresh_dashboard`、`get_settings`、`save_settings`、`set_notifications_enabled`、`send_test_notification`、`get_usage_history`、`set_history_enabled`、`clear_usage_history`、`get_diagnostics`、`get_autostart`、`set_autostart`、`open_settings_window`、`mark_main_size_manual`、`get_app_update_info`、`check_app_update` 与 `install_app_update`。`get_dashboard` 和手动刷新仅限主窗口；通知、历史、诊断、自启和更新操作仅限设置窗口；`get_settings` 是两个已知窗口共享的最小偏好读取。`dashboard-updated` 只发给主窗口，`usage-history-updated` 只发给设置窗口。更新 IPC 只返回版本摘要和下载进度；URL、签名、原始清单、凭据和未掩码账号信息永不跨越 Rust 边界。
...
```powershell
git tag -a v0.3.0 -m "v0.3.0 系统通知与本地趋势"
...
不提供未确认的静默下载或更新、任务栏停靠模式、通用旧布局迁移、云端历史同步、自定义趋势日期范围、OAuth Token 续期或重置卡操作。已有安装仅会按上述规则进行一次紧凑高度调整。
